...
class ConditionalLayerNorm(nn.Module):
    """亦可见nn.LayerNorm"""
    def __init__(self, features, embedding_dim=256, epsilon=1e-6):
        """features: normalized_shape
            epsilon: 一个很小的数，防止数值计算的除0错误
        """
        super(ConditionalLayerNorm, self).__init__()
        self.linear1=nn.Linear(embedding_dim,features)
        self.linear2=nn.Linear(embedding_dim,features)
    ...
    def forward(self, x, speaker_embedding):
        ...
        """
        L=x.shape[1]
        scale = self.linear1(speaker_embedding).unsqueeze(1).repeat(1,L,1)
        bias = self.linear2(speaker_embedding).unsqueeze(1).repeat(1,L,1)
        mean = x.mean(-1, keepdim=True)   # 在X的最后一个维度求均值
        std = x.std(-1, keepdim=True)   # 在X的最后一个维度求方差
        return scale * (x - mean) / (std + self.epsilon) + bias
